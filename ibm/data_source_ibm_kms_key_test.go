...
func TestAccIBMKMSKeyDataSource_KeyId(t *testing.T) {
	instanceName := fmt.Sprintf("kms_%d", acctest.RandIntRange(10, 100))
	// bucketName := fmt.Sprintf("bucket", acctest.RandIntRange(10, 100))
	keyName := fmt.Sprintf("key_%d", acctest.RandIntRange(10, 100))
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			resource.TestStep{
				Config: testAccCheckIBMKmsKeyDataSourceKeyIdConfig(instanceName, keyName),
				Check: resource.ComposeTestCheckFunc(
...
func testAccCheckIBMKmsKeyDataSourceKeyIdConfig(instanceName, keyName string) string {
	return fmt.Sprintf(`
